#!/usr/bin/env bash
# Valide read_track_sector FACE 1 (SEUIL piste >= TRACKS_PER_SIDE=80, = n° linéaire de
# readblock). Disque 80 pistes/face à marqueur de face (byte0 = '0'+face), lecture
# piste 3 (face 0) + piste 83 (face 1 phys. 3) -> attendu "01".
set -eu
cd "$(dirname "$0")"
...
# disque 80 pistes/face : byte0 = ord('0')+side (side-major 2x80x17x256)
python3 - "$OUT/side.raw" <<'PY'
import sys
SIDES,TRACKS,SECTORS,SECSZ=2,80,17,256
raw=bytearray()
...
PY
python3 ~/Oric1/tools/dsk_raw2mfm.py "$OUT/side.raw" "$OUT/side.dsk" sidemajor 2 80 17 >/dev/null
...
if [ "$got" = "01" ]; then
  echo "PASS: read_track_sector selectionne bien la face 1 (seuil piste >= TRACKS_PER_SIDE)"
  exit 0
else
